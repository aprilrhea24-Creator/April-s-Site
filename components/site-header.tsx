      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-3">
          {session ? (
            <>
              <Link
                href={session.role === "ADMIN" ? "/workspace" : "/client-dashboard"}
                className="rounded-full border border-cyan-300/40 px-4 py-2 text-sm text-cyan-50 transition hover:bg-cyan-300/10"
              >
                Workspace
              </Link>
              <form action="/api/auth/logout" method="POST">
                <button className="rounded-full bg-white/90 px-4 py-2 text-sm font-medium text-slate-950 transition hover:bg-cyan-100">
                  Logout
                </button>
              </form>
            </>
